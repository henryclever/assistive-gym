...
class UnpackBatchLib():
    ...
    def unpack_batch(self, batch, is_training, model, CTRL_PNL):
        ...
        if CTRL_PNL['depth_out_unet'] == True:
            ...
            INPUT_DICT['depth_noblanket_gt'] = torch.cat(
                (torch.cat((depth_noblanket[:, 0:1, :, :], depth_noblanket[:, 1:2, :, :]), dim=3),
                 torch.cat((depth_noblanket[:, 2:3, :, :], depth_noblanket[:, 3:4, :, :]), dim=3)), dim=2)
        ...
        if CTRL_PNL['recon_map_labels'] == True:
            if CTRL_PNL['recon_map_labels_test'] == True or is_training == True:
        ...
        if CTRL_PNL['depth_in'] == True:
            ...
            #print batch[depth_batch_idx].size()
            batch[-1] = torch.cat( (torch.cat((batch[-1][:, 0:1, :, :] , batch[-1][:, 1:2, :, :] ), dim = 3),
                              torch.cat((batch[-1][:, 2:3, :, :] , batch[-1][:, 3:4, :, :] ), dim = 3)), dim = 2)

            if CTRL_PNL['cal_noise'] == True and is_training == True:
                batch[-1] = PreprocessingLib().preprocessing_add_depth_calnoise(batch[-1], CTRL_PNL['cal_noise_amt'])
        ...
        if CTRL_PNL['omit_cntct_sobel'] == True or CTRL_PNL['omit_pimg_cntct_sobel'] == True:
            ...
            if CTRL_PNL['omit_pimg_cntct_sobel'] == True:
                if CTRL_PNL['cal_noise'] == True:
                    images_up[:, CTRL_PNL['num_input_channels_batch0']-1, :, :] *= 0
                else:
                    images_up[:, CTRL_PNL['num_input_channels_batch0']-2, :, :] *= 0




        if CTRL_PNL['depth_in'] == True:
            #print images_up.size(), images_up.type(),  batch[depth_batch_idx].size(), batch[depth_batch_idx].type()
            images_up = torch.cat((images_up, batch[depth_batch_idx].type(CTRL_PNL['dtype'])), dim = 1)

...
from kinematics_lib_ag import KinematicsLib
...
class CNN(nn.Module):
    def __init__(self, out_size, loss_vector_type, batch_size, verts_list, in_channels = 3):
        ...
        self.resnet = ResNet50(pretrained=False, num_classes=out_size)
        ...
        #self.resnetunet = ResNetUNet34(n_input_class = in_channels, n_scores = out_size, n_out_class = in_channels)
        ...
        self.CNN_fc1_half = nn.Sequential(
        ...
        if False:#torch.cuda.is_available():
            ...
            #self.resnet_zeros = torch.Tensor(np.zeros((128, in_channels, 128, 5))).type(torch.cuda.FloatTensor)
        else:
            self.GPU = False
            # Use for CPU
        ...
        self.meshDepthLib = MeshDepthLib(loss_vector_type, batch_size, use_cuda = self.GPU, verts_list = self.verts_list)
    ...
    def forward_kinematic_angles(self, images, gender_switch, synth_real_switch, CTRL_PNL, OUTPUT_EST_DICT,
        ...
        if CTRL_PNL['omit_cntct_sobel'] == True or CTRL_PNL['omit_pimg_cntct_sobel'] == True:
            ...
            elif CTRL_PNL['omit_pimg_cntct_sobel'] == True:
                if CTRL_PNL['cal_noise'] == True:
                    images = torch.cat((images[:, 1:CTRL_PNL['num_input_channels_batch0']-1, :, :], images[:, CTRL_PNL['num_input_channels_batch0']+1:, :, :]), dim = 1)
                else:
        ...
        if CTRL_PNL['first_pass'] == False:
            ...
            if CTRL_PNL['GPU'] == True:
                ...
                self.GPU = False
                self.dtype = torch.FloatTensor
            if CTRL_PNL['recon_map_output'] == True:
                self.verts_list = "all"
            else:
        ...
        if CTRL_PNL['CNN'] == "original":
            if CTRL_PNL['all_tanh_activ'] == True:
                if CTRL_PNL['double_network_size'] == True:
                    scores_cnn = self.CNN_packtanh_double(images)
                elif CTRL_PNL['half_network_size'] == True:
                    scores_cnn = self.CNN_packtanh_half(images)
                else:
            ...
            if CTRL_PNL['double_network_size'] == True:
                ...
                scores = self.CNN_fc1_half(scores_cnn)
            else:
            ...
            #print self.resnet._modules, 'resnet'
            #print self.resnetunet._modules, 'resnetunet'
            ...
            #    images_to_manip = newmodel(images_to_manip)#['out']
            #    print(i, images_to_manip.shape)
            #scores = self.FCN_recon(images_to_manip)

            #print images_to_manip.shape

        if scores.shape[0] == 88:
